...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || "http://localhost:5000"}/api/users/me`, {
  ...
  const handleSOS = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || "http://localhost:5000"}/api/sos/sos-call`,
        {
          location: location,
        },
        {
            ...
            <div className="space-y-6">
              {fetchingContacts ? (
                ...
                <div className="text-center py-6">
                  ...
                  <Link to="/manage-contacts" className="text-red-500 font-bold text-sm hover:underline">Add Contacts +</Link>
                </div>
              )}
            </div>
            <Link to="/manage-contacts" className="block text-center mt-10 py-4 bg-white/10 hover:bg-white/20 rounded-xl font-bold transition-all">
             Manage Circle
            </Link>
          </div>
